...
export const SETTINGS: readonly Setting[] = [
  ...
  // ── studio ────────────────────────────────────────────────────────────────
  {
    key: 'port',
    type: 'int',
    // 0 is meaningful here, not a mistake: it asks the OS for any free port,
    // which is the escape hatch when the default is taken and the answer to
    // "what if something else already has it". The URL is printed from the
    // socket afterwards, never from this value.
    min: 0,
    max: 65535,
    default: 4860,
    flag: '--port <n>',
    commands: ['studio'],
    doc: 'port to listen on; 0 picks any free port',
  },
  {
    key: 'host',
    type: 'string',
