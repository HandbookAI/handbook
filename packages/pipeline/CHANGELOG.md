...
- d7628b3: Discard "state registers" that touch fewer than two stages.
...
- 1fac45b: Fix the bundled mock LLM server truncating any path that contains a space.
...
- 6457d66: Take `organization`'s `nFunctions` from the call graph instead of the card.
