...
`--extra-body` はベンダー固有のフィールドをすべてのリクエストボディにマージします — たとえば `{"thinking":{"type":"disabled"}}`
です。model、message、token の各フィールドをこれで上書きすることはできません。
...
| `--agent-site`            | `false`           | エージェント用ロケータインデックスも `<out>/agent` の下に出力します |
...
| `--agent-dir <dir>` | —            | ロケータページを `references/agent/` の下に同梱します                    |
