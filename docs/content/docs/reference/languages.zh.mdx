...
| TypeScript _(以及 JavaScript)_ | `.ts` `.tsx` `.js` `.jsx` `.mjs` `.cjs`                  | `typescript` | `class` `interface` `enum` `alias`                   |
...
最后一列就是 `AdapterCapabilities.typeKinds`——每个 adapter **实际能找到**的，不是它想找到的。
...
这些文件每一个还会在 `phase1/scan-coverage.json` 里被单独点名，`reason: "unparsable"`，并且被挡在
图的 `scannedFiles` 之外——所以它们是一个*看得见*的缺口，而不是一个无声的缺口。
