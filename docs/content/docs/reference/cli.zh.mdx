...
`--extra-body` 会把厂商特有的字段合并进每一个请求体——比如
`{"thinking":{"type":"disabled"}}`。模型、消息和 token 相关的字段不能通过它覆盖。
...
| `--agent-site`            | `false`           | 同时生成智能体定位索引，放在 `<out>/agent`   |
...
| `--agent-dir <dir>` | —            | 把定位页面一并放进 `references/agent/`           |
