...
description: Todas las variables que Handbook lee, la regla de nombres que las genera, la cascada de .env y cuáles no deben entrar nunca en un archivo de configuración.
---
...
Seis ajustes aceptan además los nombres que la gente ya tiene exportados:
...
`llmApiKey` / `OPENAI_API_KEY` está marcada como `secret` en el registro, lo que significa
que:

- **nunca** es una bandera de línea de comandos (las banderas acaban en el historial de la
  shell y en la salida de `ps`);
- se **rechaza** si aparece en un archivo de configuración, con un mensaje que dice por
  qué — los archivos de configuración se suben al repositorio;
- se **enmascara** en la salida de `handbook config`.
