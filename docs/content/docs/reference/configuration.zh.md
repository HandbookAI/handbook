---
title: '配置参考'
description: 'Handbooks 的每一项设置，连同它的命令行参数、环境变量、配置文件键、类型与默认值——由注册表生成。'
---
...
| `llmProvider`     | `--provider <name>`         | `HANDBOOK_LLM_PROVIDER`, `OPENAI_PROVIDER`     | enum (openai\|anthropic\|gemini)            | `openai`                    | LLM 的通信格式；'openai' 覆盖所有兼容 OpenAI 的端点（绝大多数端点都是）                                                                               |
| `llmModel`        | `--model <id>`              | `HANDBOOK_LLM_MODEL`, `OPENAI_MODEL`           | string                                      | `gpt-4o-mini`               | 模型标识符                                                                                                                                            |
| `llmBaseUrl`      | `--base-url <url>`          | `HANDBOOK_LLM_BASE_URL`, `OPENAI_BASE_URL`     | string                                      | `https://api.openai.com/v1` | 任意兼容 OpenAI 的端点（托管服务、vLLM、LiteLLM、代理）；内嵌了凭据的 URL 会在配置文件中被拒绝，因为配置文件是要提交的                                |
...
| `llmExtraBody`    | —                           | `HANDBOOK_LLM_EXTRA_BODY`, `OPENAI_EXTRA_BODY` | json                                        | —                           | 合并进每个请求体的厂商字段；model/messages/token 类字段无法被覆盖。它的内容完全自由，因此按密钥对待：永远不会是命令行参数，也绝不允许出现在配置文件中 |
| `source`          | `--source <dir>`            | `HANDBOOK_SOURCE`                              | path                                        | required                    | 源码根目录；analyze/generate/plan/apply 必需，其余场景可选（validate/skill 用于哈希新鲜度判断，rollback 用于确定备份所属的文件树）                    |
| `work`            | `--work <dir>`              | `HANDBOOK_WORK`                                | path                                        | required                    | 存放流水线产物的工作目录；对 skill 可选，用于补充 coverage.json                                                                                       |
| `lang`            | `--lang <lang>`             | `HANDBOOK_LANG`                                | enum (`auto`, plus any registered language) | `auto`                      | 源码语言；auto 会检测并合并所有已注册的语言                                                                                                           |
