...
<Callout type="warn" title="`--env-file` choca con un flag de Node">
  ...
  aplica el archivo. Una ruta que existe llega a Handbook intacta, pero una ruta que **no**
...
`--extra-body` fusiona campos del proveedor en el cuerpo de cada petición — `{"thinking":{"type":"disabled"}}`,
por ejemplo. Los campos de modelo, mensajes y tokens no se pueden sobrescribir por esa vía.
...
| `--agent-site`            | `false`           | También el índice localizador para agentes, en `<out>/agent` |
...
Handbook renderizado → paquete SKILL para agentes. **Sin LLM.**
...
| `--agent-dir <dir>` | —               | Incluye las páginas localizadoras en `references/agent/`          |
...
| `--handbook <dir>` | —               | Handbook renderizado o `skills/<x>/references`. Muy recomendable |
