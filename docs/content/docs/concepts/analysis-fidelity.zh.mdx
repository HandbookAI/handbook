...
**Shell。** 含 `case` 语句的脚本会被跳过，因为那个语法会抛出异常——它的外部扫描器导入了一个固定版本 WASM 链接器不提供的符号。由于 `case` 无处不在，多数非平凡脚本都会中招（在 `nvm` 上实测：全部 6 个文件、全部 122 个函数），所以**请把 shell 的覆盖率当作部分覆盖**。扫描日志会写明原因。
