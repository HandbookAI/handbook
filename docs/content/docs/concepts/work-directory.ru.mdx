...
- **Метаданные `languages` в `graph.json`** — опциональны, потому что механизма миграции
  ...
  должен продолжать проходить валидацию. По той же причине опционально и поле
  `unparsedFiles`: его отсутствие означает, что анализ старше самой этой записи, а **не**
  то, что сбоев не было.
...
`_coverage.json` отвечает на вопрос _«какие файлы модель не смогла описать?»_.
`scan-coverage.json` отвечает на вопрос, лежащий уровнем ниже: _«какие файлы парсер вообще
не сумел прочитать?»_ — с `reason`, равным `unreadable`, `unparsable` или `partial`.
...
Handbooks пишет за пределами рабочего каталога ровно в двух местах, оба — по явному выбору
