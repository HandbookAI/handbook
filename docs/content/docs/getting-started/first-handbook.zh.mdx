...
<Callout type="warn" title="继续之前先读懂这些数字">
  ...
  **`files` 高得离谱？** 你正在分析 `node_modules`、`vendor`
  ...
  `phase1/dropped-calls.json`——每一个未解析的调用都在那里被分了类，而不是被藏起来。 - **`filesUnparsed` 不是
  0？** 这些文件都带着原因写在 `phase1/scan-coverage.json` 里。其中 `unreadable` 和 `unparsable`
  的那些什么事实都贡献不了，也不会有自己的页面，所以现在生成出来的手册，恰恰在那里有个洞——值得在为文字付钱之前先补上。
</Callout>
