...
<Callout type="warn" title="Segredos são rejeitados aqui">
  `llmApiKey` / `OPENAI_API_KEY` nunca devem aparecer em um arquivo de configuração — arquivos de configuração
  acabam commitados. O carregador recusa o arquivo de imediato e diz por quê. Coloque o segredo no `.env` ou
