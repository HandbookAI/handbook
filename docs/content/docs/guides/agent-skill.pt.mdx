...
| `--agent-dir`         | Inclui as páginas localizadoras e adiciona um passo de desambiguação ao protocolo de roteamento |
...
4. Consulte `references/registers.md` para o estado transversal — inestimável em mudanças
   com fan-out.
5. _(com `--agent-dir`)_ Desambigue quando um termo for ambíguo.
...
- **As páginas localizadoras são incluídas em par ou não são incluídas.** O `SKILL.md`
  nunca deve rotear para um arquivo que não está lá.
