...
| `--agent-dir`         | Ships the locator pages, and adds a disambiguation step to the routing protocol |
...
5. _(with `--agent-dir`)_ Disambiguate when a term is ambiguous.
...
- **Locator pages ship as a pair or not at all.** `SKILL.md` must never route to a file
  that is not there.
