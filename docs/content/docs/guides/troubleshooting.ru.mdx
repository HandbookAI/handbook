...
Это вообще не ошибка Handbook. Node >= 20.6 имеет собственный флаг `--env-file` и заранее
сканирует всю командную строку в его поисках, поэтому умирает на отсутствующем пути
**до** старта Handbook. Используйте переменную — её ничто не может перехватить:
