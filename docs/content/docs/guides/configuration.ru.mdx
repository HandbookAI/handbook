...
<Callout type="warn" title="Секреты здесь отклоняются">
  `llmApiKey` / `OPENAI_API_KEY` никогда не должны появляться в конфигурационном файле — конфигурационные
  файлы попадают в коммиты. Загрузчик наотрез отказывается от такого файла и объясняет почему. Положите ключ в
  `.env` или в окружение оболочки.
</Callout>
