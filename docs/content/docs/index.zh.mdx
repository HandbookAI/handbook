...
<div className="not-prose mb-8">
  <img
    src="/diagrams/hero.zh.svg"
    alt="Handbooks —— 一个代码库进去，两本手册出来：一本团队读的叙述式文档站，一份智能体用来定位的机器可读索引"
...
Handbooks 会把同一张代码地图写成**两份**，因为它有两种完全不同的读者：
...
Handbooks 用 [tree-sitter](https://tree-sitter.github.io/) 解析每一个源文件，构建一张带类型的调用图：函数、方法、通过 `self`/属性/参数/导入解析出来的调用边、离开你代码的调用，以及无法解析的调用——后者被隔离进单独的文件，绝不靠猜。
