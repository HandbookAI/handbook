...
- **生成散文的质量评估**：手册里人读的那部分是 LLM 写的，离线 mock 只产出占位文本，
  所以「散文准确率」无法离线评判。要评估必须对真实端点跑 `demo:self:real` 或对某个真实仓库
  跑一次带 key 的 generate——**那会花你的 token，所以我没有擅自跑**。
  确定性的那一半（agent 定位索引、结构、调用事实、覆盖率）已全部评估并修完。
